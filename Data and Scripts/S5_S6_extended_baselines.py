#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Additional parsimonious small-sample baselines (ridge, elastic net, GPR, SVR).

Reviewer 4 (Revision 1), comment 2.
...
Run:  python run_extra_baselines.py
...
MODELS = [
    ...
    ("Ridge regression",
     ...
     {"m__alpha": np.logspace(-3, 3, 13)}),
    ("Elastic net",
     Pipeline([("sc", StandardScaler()), ("m", ElasticNet(max_iter=200000))]),
     {"m__alpha": np.logspace(-3, 2, 11), "m__l1_ratio": [0.1, 0.3, 0.5, 0.7, 0.9, 1.0]}),
    ...
    ("Support vector regression (RBF kernel)",
     ...
     {"m__regressor__C": np.logspace(-1, 3, 9), "m__regressor__gamma": np.logspace(-3, 1, 9),
      "m__regressor__epsilon": [0.01, 0.1, 0.3]}),
]
...
if __name__ == "__main__":
    ...
    for name, est, grid in MODELS:
        ...
        for t in TARGETS:
            ...
            met.append(dict(model=name, target=t, **metrics(y, p)))
    for name in abs_err:
        paired_vs_llm(D, abs_err[name], name, tests)
    llm = llm_sample_abs_errors(D)
    for (m, t), (ae, pm) in llm.items():
        met.append(dict(model=m, target=t, **metrics(D[t].to_numpy(float), pm)))
    pd.DataFrame(met).round(4).to_csv("../Supplementary Materials/Table_S5_extended_baselines.csv", index=False)
    pd.DataFrame(tests).round(4).to_csv("../Supplementary Materials/Table_S6_paired_tests_extended_baselines.csv", index=False)
